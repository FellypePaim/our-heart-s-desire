...
const BillingRules = () => {
  ...
  // Fetch logs for expanded rule
  const { data: ruleLogs = [] } = useQuery({
    queryKey: ["billing_rule_logs", expandedRule],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("billing_rule_logs")
        .select("*")
        .eq("rule_id", expandedRule!)
        .order("executed_at", { ascending: false })
        .limit(20);
      if (error) throw error;
      return (data || []) as RuleLog[];
    },
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <CollapsibleTrigger asChild>
                        <Button variant="ghost" size="icon" title="Histórico de execuções">
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  ...
                                  <Badge
                                    ...
                                    className="text-xs"
                                  >
                                    {log.status === "completed" ? "Sucesso" : log.status === "error" ? "Erro" : "Sem correspondências"}
                                  </Badge>
                                </div>
                                ...
                                <div className="flex gap-4 text-xs text-muted-foreground">
                                  <span className="flex items-center gap-1">
                                    <Users className="h-3 w-3" />
                                    {log.clients_matched} encontrados
                                  </span>
                                  <span className="flex items-center gap-1">
                                    <CheckCircle className="h-3 w-3 text-green-500" />
                                    {log.messages_sent} enviadas
                                  </span>
                                  {log.messages_failed > 0 && (
                                    <span className="flex items-center gap-1">
                                      <XCircle className="h-3 w-3 text-destructive" />
                                      {log.messages_failed} falharam
                                    </span>
                                  )}
                                </div>
                                  ...
                                  <div className="rounded bg-destructive/10 border border-destructive/20 p-2 text-xs space-y-1">
                                    {(log.errors as any[]).slice(0, 3).map((err: any, i: number) => (
                                      <p key={i} className="text-destructive">
                                        {err.client_name || "Cliente"}: {err.error || "Erro desconhecido"}
                                      </p>
                                    ))}
                                    {(log.errors as any[]).length > 3 && (
                                      <p className="text-muted-foreground">
                                        +{(log.errors as any[]).length - 3} erros adicionais
                                      </p>
                                    )}
                                  </div>
                                )}
